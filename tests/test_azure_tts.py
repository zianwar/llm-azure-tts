...
def test_azure_tts(httpx_mock, tmp_path):
    ...
    assert request.url == "https://api.example.com/openai/deployments/tts-1/audio/speech?api-version=2024-02-15"
...
def test_azure_tts_rate_limit(httpx_mock, tmp_path):
    ...
    assert "Rate limit exceeded. Need to wait for 30 seconds before retrying." in result.output